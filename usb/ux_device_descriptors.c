...
char *ux_device_sn_text(void)
{
    ...
    // create similar serial number as in obsolete USB library "STM32_USB_Device_Library"
    sn0 = LL_GetUID_Word0();
    sn1 = LL_GetUID_Word1();
    sn2 = LL_GetUID_Word2();
    sn0 += sn2;

    snprintf(buf, sizeof(buf), "%04lx%08lx", ((sn1 >> 16) ^ sn1) & 0xFFFF, sn0);
    return (buf);
}
#define USBD_SERIAL_NUMBER ux_device_sn_text()
